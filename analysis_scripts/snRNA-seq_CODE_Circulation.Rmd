...
```{r}
#Function to just create the objects and violin plot so then we can filter them accordingly.
...
#We need to normalize and dim reduction to run Doublet finder. This needs to be done if not doublet finder doesnt work.
```{r}
...
#Now we will run the doubletfinder removal package.
```{r}
...
#Now lets remove the cells identified as doublets.
```{r}
...
#Now create RISC objects to run integration.
```{r}
...
#Lets normalize and find variable genes.
```{r}
...
```
#Following RISC instructions we decide Heart1n2 woukd be the reference.
...
#Since we have already orig.indentity column from Seurat, lets rename values in Set (from RISC) and create a condition colunm.
```{r}
...
#Run UMAP reduction and clustering and plot
```{r}
...
#Since we have a separated cluster (of 31 cells) we need to remove it from the beginning and rerun everything. Will do is manually give them a new cluster assignation based on umap coordinates.
```{r}
...
#Just create a new object (we could keep working on the original but just in case).
```{r}
Object_RISC2 <- Object_RISC
```

#Lets subset from Object_RISC2.
```{r}
...
#Run DE as pseudo-bulk RNAseq using edgeR. For that create a new count table summing cells coming from same sample. Since RISC doesnt save raw data I need to go back to seurat and subset the data again.
...
#Now create the metadata table to create edgeR object.
```{r}
...
# 2 UMAP fromc ontrols samples and infarts.
```{r}
...
#We need information of UMAP coordinates to run scvelo and cellrank.
```{r}
